...
module.exports = function(grunt) {
  ...
  var _           = require('lodash')
    ...
    , DEPRECATED  = require('./data-deprecated-hypr.js')
    , temp

    , jsonify     = require('./helpers.js').jsonify
    ;
  ...
  , function() {
      ...
      , regex = {
        ...
        , HYPR_DEPRECTATED_TAGS: new RegExp(['(?:\{\%|\{\{)\s*(', DEPERECATED.TAGS.join('|'), ')'].join(''), 'gi')
        , HYPR_DEPRECTATED_FILTERS: new RegExp(['\|(', DEPRECATED.FILTERS.join('|'), ')'].join(''), 'gi')
      ...
      _.each(templates, function(filepath, index) {
        ...
        _.each(file.content, function(line, number) {
          ...
          if (line.match(regex.HYPR_DEPRECATED_FILTER_FORMAT)) {
            ...
            grunt.log.error(MESSAGES.DEPRECATED_FILTER, newLine, number);
